...
class TodoList:
    ...
    def count_uncompleted_items(self):
        return sum(1 for item in self.items if not item.completed)


host = "localhost"
port = 9990  # port number to listen on
...
while True:
    ...
    if choice == "1":
        ...
        uncomplete_count = todo_list.count_uncompleted_items()
        result = f"Uncompleted todos: {uncomplete_count}"
    else:
